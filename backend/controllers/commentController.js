...
const getResearchPaperComments = async (req, res) => {
  ...
  if (!comments || comments.length < 1) {
    return res
      .status(400)
      .json({ message: "No comments for given research paper found" });
  }
  return res.status(200).json({ comments, total: comments.length });
};
...
const createComment = async (req, res) => {
  ...
  if (!newComment) {
    return res
      .status(500)
      .json({ message: "Error while creating new Comment" });
  }
  return res
    .status(200)
...
const updateComment = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Comment updated successfully", comment: commentExist });
};
...
const deleteComment = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Comment Deleted successfully,", comment: commentExist });
};
module.exports = {
  getComments,
  getSingleComment,
