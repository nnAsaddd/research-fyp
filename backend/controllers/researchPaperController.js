...
const getResearchPapers = async (req, res) => {
  ...
  return res.status(200).json({
    message: "Research papers found",
    researchPapers: papers,
    total: papers.length,
  });
};

const getSingleResearchPaper = async (req, res) => {
  const { id } = req.params;
  const researchPaper = await ResearchPaper.findOne({ _id: id });
  if (!researchPaper) {
    return res.status(400).json({ message: `No Research Paper with id found` });
  }
...
const createResearchPaper = async (req, res) => {
  ...
  if (!req.file) {
    return res
      .status(400)
      .json({ message: "Please send data in form-data form" });
  }
  ...
  if (!name || !collectionId) {
    return res
      .status(400)
      .json({ message: "Please provide with all the necessary information." });
  }
  const collectionExist = await Collection.findOne({ _id: collectionId });
  if (!collectionExist) {
    return res
      .status(400)
      .json({ message: "No collection with given id found" });
  }
  ...
  if (!newPaper) {
    return res
      .status(500)
      .json({ message: "Error creating new research paper" });
  }
  return res.status(200).json({
    message: "Research paper successfully created",
    researchPaper: newPaper,
  });
};
...
const deleteResearchPaper = async (req, res) => {
  ...
  if (!paper) {
    return res
      .status(500)
      .json({ message: "Error while deleting research paper" });
  }
  ...
  try {
    ...
    console.log("Error while deleting the pdf file" + err);
  }
  return res.status(200).json({
...
const updateResearchPaper = async (req, res) => {
  const { name, physicalLocation, id } = req.body;
  if (!id) {
    return res
      .status(400)
      .json({ message: "Please provide research paper id" });
  }
  const paper = await ResearchPaper.findOne({ _id: id });
  if (!paper) {
...
const getCollectionResearchPaper = async (req, res) => {
  ...
  if (!collectionId) {
    return res
      .status(400)
      .json({ message: "Please provide collection id found" });
  }
  ...
  if (!papers || papers.length < 1) {
    return res
      .status(400)
      .json({ message: "No papers for the given collection found" });
  }
